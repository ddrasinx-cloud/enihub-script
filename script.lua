...
local function drawKeyUI(vp)
    ...
    keyBg.Size = vp; keyBg.Visible = true
    keyTitle.Visible = true; keyTitle.Position = Vector2.new(cx, cy-80)
    keySub.Visible = true; keySub.Position = Vector2.new(cx, cy-50)
    keyBox.Visible = true; keyBox.Position = Vector2.new(cx-170, cy-10); keyBox.Size = Vector2.new(340, 50)
    keyLine.Visible = true; keyLine.Position = Vector2.new(cx-170, cy+38); keyLine.Size = Vector2.new(340, 2)
    keyInput.Visible = true; keyInput.Position = Vector2.new(cx, cy+8); keyInput.Text = keyStr
    keyCursor.Visible = true; keyCursor.Position = Vector2.new(cx+(#keyStr*7), cy+8)
    keyStatus.Visible = true; keyStatus.Position = Vector2.new(cx, cy+60)
    keyHint.Visible = true; keyHint.Position = Vector2.new(cx, cy+120)
end
...
local fovOn = false; local radarOn = false; local uiOpen = true
...
local function getEsp(i)
    while #espPool < i do
        table.insert(espPool, {
            box = d("Square", {Color=Color3.fromRGB(0,180,210),Thickness=1.5,Filled=false,Transparency=1}),
            fill = d("Square", {Color=Color3.new(0,0,0),Filled=true,Transparency=0.55}),
            nam = d("Text", {Color=Color3.fromRGB(220,220,230),Size=13,Center=true,Outline=true,OutlineColor=Color3.new(0,0,0)}),
            dist = d("Text", {Color=Color3.fromRGB(140,140,150),Size=11,Center=true,Outline=true,OutlineColor=Color3.new(0,0,0)}),
            ...
            tr = d("Line", {Color=Color3.fromRGB(0,180,210),Thickness=1,Transparency=1})
        })
    end
    return espPool[i]
end
local function hideEsp(i)
    local e = espPool[i]; if not e then return end
    e.box.Visible = false; e.fill.Visible = false; e.nam.Visible = false
    e.dist.Visible = false; e.hp.Visible = false; e.hpb.Visible = false; e.tr.Visible = false
end
...
local function getRD(i)
    while #rDots < i do table.insert(rDots, {dot=d("Square", {Color=Color3.fromRGB(255,50,50),Size=Vector2.new(4,4),Filled=true,Transparency=1}),lbl=d("Text", {Color=Color3.fromRGB(255,255,255),Size=9,Center=true,Outline=true,OutlineColor=Color3.new(0,0,0)})}) end
    return rDots[i]
end
            ...
            if p ~= lp then
                ...
                if alive then
                    ...
                    if on and sp.Z>0 then
                        ...
                        local bh = math.clamp(400/dist*size.Y*1.5,20,200); local bw = bh*0.6; local pos = Vector2.new(sp.X-bw/2, sp.Y-bh/2)
