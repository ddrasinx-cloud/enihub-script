...
local function c(obj, ev, fn)
  local con = obj[ev]:Connect(fn)
...
local K = {
  ...
  961987163, 1508970993, 2453635748, 2870763222,
  3624381080, 310598401, 607225278, 1426881987,
  1925078388, 2162078206, 2614888103, 3248222580,
  3835390401, 4022224774, 264347078, 604807628,
  770255983, 1249150122, 1555081692, 1996064986,
  2554220882, 2821834349, 2952996808, 3210313671,
  3336571891, 3584528711, 113536993, 259148383,
  359350499, 404831999, 641693614, 528389916,
  980424654, 1403092006, 2005096759, 2085556316,
  2522072916, 2797796833, 3103778035, 3496256113,
  3637589316, 165732119, 262161324, 290432414,
  489992340, 796298094, 1086882520, 1299590309,
  1708972127, 2118057954, 2406982945, 2694243992,
  3381252166, 3661272398, 446989822, 694788565,
  1069398592, 1441815577, 1509267071, 2029154682
}
...
local function sha256(msg)
  ...
  for i = 7, 0, -1 do
    msg[#msg + 1] = band(rshift(ml, i * 8), 255)
  end
  for i = 1, #msg, 64 do
    local w = {}
    for j = 1, 16 do
      w[j] = 0
      for k = 1, 4 do
        w[j] = bor(lshift(w[j], 8), msg[i + (j - 1) * 4 + k - 1] or 0)
      end
    end
    for j = 17, 64 do
      ...
      w[j] = band(w[j - 16] + s0 + w[j - 7] + s1, 4294967295)
    end
    local a = H[1]
    local b = H[2]
    local c = H[3]
    ...
    for j = 1, 64 do
      ...
      local t1 = band(h + S1 + ch + K[j] + w[j], 4294967295)
      local S0 = bxor(bxor(ror(a, 2), ror(a, 13)), ror(a, 22))
      local maj = bxor(bxor(band(a, b), band(a, c)), band(b, c))
      local t2 = band(S0 + maj, 4294967295)
      h = g
      g = f
      f = e
      e = band(d + t1, 4294967295)
      d = c
      c = b
      b = a
      a = band(t1 + t2, 4294967295)
    end
    H[1] = band(H[1] + a, 4294967295)
    H[2] = band(H[2] + b, 4294967295)
    H[3] = band(H[3] + c, 4294967295)
    H[4] = band(H[4] + d, 4294967295)
    H[5] = band(H[5] + e, 4294967295)
    H[6] = band(H[6] + f, 4294967295)
    H[7] = band(H[7] + g, 4294967295)
    H[8] = band(H[8] + h, 4294967295)
  end
  local out = {}
  for i = 1, 8 do
    for j = 3, 0, -1 do
      out[#out + 1] = band(rshift(H[i], j * 8), 255)
    end
  end
  return out
end
...
local function hmacSHA256(msg, secret)
  ...
  local ipad = {}
  for i = 1, 64 do
    opad[i] = bxor(key[i], 92)
    ipad[i] = bxor(key[i], 54)
  end
  for i = 1, #msg do
    ipad[#ipad + 1] = msg:byte(i)
  end
...
local function base64decode(s)
  ...
  for i = 1, #s, 4 do
    ...
    for j = 1, 4 do
      ...
      local idx = 0
      if c ~= "=" then
        idx = b64chars:find(c, 1, true) - 1
      end
      v = v * 64 + idx
    end
    out[#out + 1] = band(rshift(v, 16), 255)
    out[#out + 1] = band(rshift(v, 8), 255)
    out[#out + 1] = band(v, 255)
  end
  local t = {}
  for i = 1, #out do
    if out[i] then
      t[#t + 1] = string.char(out[i])
    end
  end
  return table.concat(t)
end
...
local function validateKey(key)
  ...
  if exp then
    ...
    if e and e > 0 and e < os.time() * 1000 then
      return false
    end
  end
  return true
...
local function drawKey(vp)
  ...
  local py = cy - ph / 2
  k.overlay.Size = vp
  k.overlay.Visible = true
  k.bg.Position = Vector2.new(px, py)
  k.bg.Size = Vector2.new(pw, ph)
  k.bg.Visible = true
  k.border.Position = Vector2.new(px, py)
  k.border.Size = Vector2.new(pw, ph)
  k.border.Visible = true
  k.title.Position = Vector2.new(cx, py + 28)
  k.title.Visible = true
  k.sub.Position = Vector2.new(cx, py + 56)
  k.sub.Visible = true
  local ibx = cx - 150
  local iby = py + 76
  local ibw = 300
  local ibh = 44
  k.inBg.Position = Vector2.new(ibx, iby)
  k.inBg.Size = Vector2.new(ibw, ibh)
  k.inBg.Visible = true
  k.inBorder.Position = Vector2.new(ibx, iby)
  k.inBorder.Size = Vector2.new(ibw, ibh)
  k.inBorder.Visible = true
  k.inText.Text = keyStr
  k.inText.Position = Vector2.new(cx, iby + ibh / 2 - 6)
  k.inText.Visible = true
  local tw = k.inText.TextBounds.X
  k.cursor.Visible = blink
  k.cursor.Position = Vector2.new(cx + tw / 2 + 6, iby + ibh / 2 - 6)
  k.status.Position = Vector2.new(cx, py + 132)
  k.status.Visible = true
  k.hint.Position = Vector2.new(cx, py + 240)
  k.hint.Visible = true
end
  ...
  if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode ~= Enum.KeyCode.Unknown then
    ...
    if kc == Enum.KeyCode.Return or kc == Enum.KeyCode.KeypadEnter then
      ...
      if validateKey(keyStr) then
        k.status.Text = "Key accepted"
        k.status.Color = c_green
        task.wait(0.5)
        authenticated = true
        k.status.Text = ""
      else
        k.status.Text = "Invalid key"
        k.status.Color = c_red
        keyStr = ""
      end
      return
    end
...
local function getEsp(i)
  while #espPool < i do
    table.insert(espPool, {
      box = d("Square", {Color = c_accent, Thickness = 1.5, Filled = false, Transparency = 1}),
      fill = d("Square", {Color = Color3.new(0, 0, 0), Filled = true, Transparency = 0.55}),
      nam = d("Text", {Color = c_text, Size = 13, Center = true, Outline = true, OutlineColor = Color3.new(0, 0, 0)}),
      dist = d("Text", {Color = c_sub, Size = 11, Center = true, Outline = true, OutlineColor = Color3.new(0, 0, 0)}),
      hp = d("Square", {Color = Color3.fromRGB(55, 200, 95), Filled = true, Transparency = 1}),
      hpb = d("Square", {Color = Color3.fromRGB(30, 30, 30), Filled = true, Transparency = 1}),
      tr = d("Line", {Color = c_accent, Thickness = 1, Transparency = 1})
    })
  end
  return espPool[i]
end

local function hideEsp(i)
  local e = espPool[i]
  if not e then
    return
  end
...
local function getRD(i)
  while #rDots < i do
    table.insert(rDots, {
      dot = d("Square", {Color = Color3.fromRGB(255, 50, 50), Size = Vector2.new(4, 4), Filled = true, Transparency = 1}),
      lbl = d("Text", {Color = Color3.fromRGB(255, 255, 255), Size = 9, Center = true, Outline = true, OutlineColor = Color3.new(0, 0, 0)})
    })
  end
  return rDots[i]
end

local function hideRD(i)
  local r = rDots[i]
  if r then
    r.dot.Visible = false
    r.lbl.Visible = false
...
local waitCon = RunService.RenderStepped:Connect(function()
  ...
  local togDefs = {
    {name = "Aimbot", key = "aim", y = 42},
    {name = "ESP", key = "esp", y = 68},
    {name = "Tracers", key = "trace", y = 94},
    {name = "FOV Circle", key = "fov", y = 120},
    {name = "Radar", key = "radar", y = 146}
  }
  local toggles = {}
  ...
  local function menuHit(x, y, w, h)
  ...
  local function drawMenu()
    ...
    m.bg.Position = Vector2.new(mx, my)
    m.bg.Size = Vector2.new(mw, mh)
    m.border.Position = Vector2.new(mx, my)
    m.border.Size = Vector2.new(mw, mh)
    m.title.Position = Vector2.new(mx + 14, my + 7)
    m.ver.Position = Vector2.new(mx + mw - 32, my + 9)
    m.cl.Position = Vector2.new(mx + mw - 14, my + 7)
    m.fovLbl.Position = Vector2.new(mx + 14, my + mh - 42)
    m.fovHint.Position = Vector2.new(mx + mw - 30, my + mh - 44)
    m.exitHint.Position = Vector2.new(mx + mw / 2, my + mh - 12)
    for _, td in ipairs(togDefs) do
      local t = toggles[td.key]
      local bx = mx + mw - 52
      local by = my + td.y
      t.bg.Position = Vector2.new(bx, by)
      t.bg.Size = Vector2.new(40, 18)
      t.lbl.Position = Vector2.new(mx + 14, my + td.y)
      t.txt.Position = Vector2.new(bx + 20, by + 6)
      if not t.on then
        local hov = menuHit(bx, by, 40, 18)
        t.bg.Color = hov and c_btnHov or c_btnOff
      end
    end
    if menuHit(mx + mw - 24, my + 2, 20, 20) then
      m.cl.Color = c_text
    else
      m.cl.Color = c_sub
    end
  ...
  c(UIS, "InputBegan", function(input)
    ...
    if input.KeyCode == Enum.KeyCode.Left or input.KeyCode == Enum.KeyCode.Comma then
      aimFov = math.max(50, aimFov - 10)
      m.fovLbl.Text = "FOV: " .. math.floor(aimFov)
    end
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 and menuOpen then
      ...
      if menuHit(mx, my, mw, 28) then
        dragging = true
        dragOff = Vector2.new(mp.X - mx, mp.Y - my)
      end
      if menuHit(mx + mw - 24, my + 2, 20, 20) then
        menuOpen = false
        return
      ...
      for _, td in ipairs(togDefs) do
        ...
        if menuHit(bx, by, 40, 18) then
          ...
          if td.key == "aim" then
            aimOn = t.on
          elseif td.key == "esp" then
            espOn = t.on
          elseif td.key == "trace" then
            traceOn = t.on
          elseif td.key == "fov" then
            fovOn = t.on
          elseif td.key == "radar" then
            radarOn = t.on
          end
        end
      end
  ...
  c(UIS, "InputEnded", function(input)
  ...
  c(RunService, "RenderStepped", function()
    ...
    cx.h.Visible = true
    cx.dot.Visible = true
    cx.v.From = Vector2.new(mid.X, mid.Y - 8)
    cx.v.To = Vector2.new(mid.X, mid.Y + 8)
    cx.h.From = Vector2.new(mid.X - 8, mid.Y)
    cx.h.To = Vector2.new(mid.X + 8, mid.Y)
    ...
    if aimOn and UIS:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
      ...
      if best then
        Camera.CFrame = Camera.CFrame:Lerp(CFrame.lookAt(Camera.CFrame.Position, best.Position), smooth)
      end
    end
    if radarOn then
      rBg.Visible = true
      ...
      local rx = vp.X - 145
      local ry = vp.Y - 145
      rBg.Position = Vector2.new(rx, ry)
      rBg.Size = Vector2.new(130, 130)
      rR1.Position = Vector2.new(rx + 65, ry + 65)
      rR2.Position = Vector2.new(rx + 65, ry + 65)
      rDot.Position = Vector2.new(rx + 63, ry + 63)
      local rdIdx = 0
      local lpc = lp.Character
      local lpr = nil
      if lpc then
        lpr = lpc:FindFirstChild("HumanoidRootPart")
      end
      if lpr then
        local lpos = lpr.Position
        ...
        for _, p in pairs(Players:GetPlayers()) do
          if p ~= lp then
            ...
            if c2 and c2:FindFirstChild("HumanoidRootPart") and c2:FindFirstChild("Humanoid") and c2.Humanoid.Health > 0 then
              local r2 = c2.HumanoidRootPart
              local rel = (r2.Position - lpos) * Vector3.new(1, 0, 1)
              local dist = rel.Magnitude
              if dist < 300 then
                local ang = math.atan2(rel.X, rel.Z) - yaw
                local rd = math.min(dist / 300 * 60, 58)
                local dx = math.sin(ang) * rd
                local dy = math.cos(ang) * rd
                rdIdx = rdIdx + 1
                local o = getRD(rdIdx)
                o.dot.Visible = true
                o.dot.Position = Vector2.new(rx + 63 + dx, ry + 63 + dy)
                o.lbl.Visible = true
                o.lbl.Position = Vector2.new(rx + 63 + dx, ry + 63 + dy - 8)
                o.lbl.Text = p.Name:sub(1, 8)
              end
            end
          end
        end
      end
      for i = rdIdx + 1, #rDots do
        hideRD(i)
      end
    else
      rBg.Visible = false
    ...
    for _, p in pairs(Players:GetPlayers()) do
      if p ~= lp then
        ...
        local root = nil
        if c2 then
          hum = c2:FindFirstChild("Humanoid")
          root = c2:FindFirstChild("HumanoidRootPart")
        end
        ...
        if alive then
          ...
          if on and sp.Z > 0 then
            ...
            local size = c2:GetExtentsSize()
            local bh = math.max(20, math.min(200, 400 / dist * size.Y * 1.5))
            local bw = bh * 0.6
            local pos = Vector2.new(sp.X - bw / 2, sp.Y - bh / 2)
            e.box.Visible = espOn
            e.box.Position = pos
            e.box.Size = Vector2.new(bw, bh)
            e.fill.Visible = espOn
            e.fill.Position = pos
            e.fill.Size = Vector2.new(bw, bh)
            local hp = hum.Health / hum.MaxHealth
            local hc = Color3.fromRGB(60 * (1 + (1 - hp)), 200 * hp, 60 * hp)
            e.hpb.Visible = espOn
            e.hpb.Position = Vector2.new(pos.X - 5, pos.Y)
            e.hpb.Size = Vector2.new(3, bh)
            e.hp.Visible = espOn
            e.hp.Position = Vector2.new(pos.X - 5, pos.Y + bh * (1 - hp))
            e.hp.Size = Vector2.new(3, bh * hp)
            e.hp.Color = hc
            local nm = p.Name
            if #nm > 12 then
              nm = nm:sub(1, 12) .. ".."
            ...
            e.nam.Visible = espOn
            e.nam.Position = Vector2.new(sp.X, pos.Y - 15)
            e.nam.Text = nm
            e.dist.Visible = espOn
            e.dist.Position = Vector2.new(sp.X, pos.Y + bh + 2)
            e.dist.Text = math.floor(dist) .. "m"
            if traceOn and espOn then
              e.tr.Visible = true
              e.tr.From = Vector2.new(vp.X / 2, vp.Y)
              e.tr.To = Vector2.new(sp.X, sp.Y)
            else
              e.tr.Visible = false
            end
          end
